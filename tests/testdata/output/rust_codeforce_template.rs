pub mod pr {
    ...
    #[inline(always)]
    #[allow(dead_code)]
    pub fn pelem<T: std::fmt::Display>(x: &T) {
        print!("{}", x)
    }
    #[inline(always)]
    #[allow(dead_code)]
...
#[macro_export]
macro_rules ! scan (($ text : expr , $ ($ arg : expr) ,*) => { use :: std :: io :: Read ; $ crate :: scan ! (:: std :: io :: stdin () . bytes () . map (std :: result :: Result :: unwrap) => $ text , $ ($ arg) ,*) ; format_args ! ($ text , $ ($ arg) ,*) ; } ; ($ input : expr => $ pattern : expr , $ ($ arg : expr) ,*) => { { $ crate :: try_scan ! (@ impl unwrap ; $ input => $ pattern , $ ($ arg) ,*) } } ;) ;
fn solve() {}
fn main() {
